...
// Import the Flipkart search API
import flipkartSearch from '../utils/flipkartSearch';

const FlipkartSearchScreen = ({ route, navigation }) => {
  const { searchQuery } = route.params || {};
  const webViewRef = useRef(null);
  const [status, setStatus] = useState('Loading Flipkart...');
  ...
  // Helper function to check if a URL is a Flipkart search results page
  const isSearchResultsPage = (url) => {
    // Flipkart search results usually have /search? in the URL path
  ...
  const handleMessage = (event) => {
    try {
      ...
      else if (data.type === 'flipkart_search_results' || data.type === 'products') {
      ...
      setError('Failed to process data from Flipkart');
      setIsLoading(false);
    }
  };

  // Save product data to a file
  const saveProductsToFile = async (products, filePrefix = 'flipkart_products') => {
    try {
  ...
  const handleWebViewLoadEnd = () => {
    ...
    setStatus('Flipkart loaded, starting search...');
    
    // Use our utility function to search Flipkart
  ...
  return (
    <SafeAreaView style={CommonStyles.container}>
      ...
      <View style={CommonStyles.header}>
        ...
        <Text style={CommonStyles.headerTitle}>Flipkart Search</Text>
        ...
        <>
          ...
          {!searchComplete && (
            <View style={{ flex: 1 }}>
              <WebView
                ref={webViewRef}
                source={{ uri: flipkartSearch.getUrl(searchQuery) }}
...
export default FlipkartSearchScreen;
